    ...
    public function checkout()
    {
        ...
        if ($PluginResult) {
            ...
            if ($this->securityUtil->checkMultiAtack()) {
                // 不正利用の場合
                $message = trans('yamato_payment.shopping.checkout.invalid');
                $messages[] = $message;
                $this->securityUtil->setErrorMessage($message);
                $this->securityUtil->removeMultiAtack();
                ...
                // リダイレクト
                $redirectResponse = new RedirectResponse($this->router->generate('shopping_error', [], UrlGeneratorInterface::ABSOLUTE_URL));
                $result->setResponse($redirectResponse);
            } else {
                $messages[] = trans('yamato_payment.shopping.checkout.error');
