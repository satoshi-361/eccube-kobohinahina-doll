...
class YamatoStatusService
{
    ...
    public function doChangeDeferredPaymentStatus($Orders, $mode)
    {
        ...
        foreach ($Orders as $Order) {
            ...
            if (!$ret) {
                return $this->getErrorWithPaymentOperationsForDeferred($orderId, $count);
            }
        }
        return [$count, $listErrorMsg];
    }
     ...
     * @return array 件数, エラー情報
     */
    public function getErrorWithPaymentOperationsForDeferred($orderId, $count)
    {
        $listErrorMsg[] = '※ 決済操作エラー が発生しました。';
        $listErrorMsg[] = '注文番号 ' . $orderId;
        $errors = $this->deferredClientService->getError();
        foreach ($errors as $error) {
            $listErrorMsg[] = $error;
        }
        $this->deferredClientService->clear();
        return [$count, $listErrorMsg];
    }
