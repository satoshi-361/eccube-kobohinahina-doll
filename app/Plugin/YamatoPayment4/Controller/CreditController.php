...
class CreditController extends AbstractController
{
    ...
    public function index(Request $request)
    {
        ...
        // 3Dセキュア画面からの戻り
        if ($request->query->get('mode') == '3dTran') {
            $orderId = $request->query->get('orderId');
            $dummyOrder = $this->orderRepository->findOneBy(['id' => $orderId]);
            $listParam = $request->request->all();

            $this->creditClientService->setPaymentInfo($paymentInfo);

            // 決済実行 ACSからの戻りは加工せずに戻す
            if (empty($listParam['TOKEN'])) {
                $result = $this->creditClientService->doSecureTran($dummyOrder, $listParam);
            } else {
                $result = $this->creditClientService->doSecureTranToken($dummyOrder, $listParam);
            }
            ...
            if ($result === false) {
                ...
                // 登録のために行った1円決済をキャンセル
                $cancelResult = $this->creditClientService->doCancelDummyOrder($dummyOrder);
    ...
    public function register(Request $request)
    {
        ...
        $pluginData = $this->creditClientService->getSendResults();
        ...
        if ($result === false) {
            ...
            if (isset($pluginData['threeDAuthHtml'], $pluginData['threeDToken'])) {
                ...
                $yamatoOrder->setOrder($dummyOrder);
                $this->entityManager->persist($this->creditClientService->setOrderPayData($yamatoOrder, $pluginData));
                $this->entityManager->flush();

                $PaymentResult = new PaymentResult();
                $PaymentResult->setSuccess(true);
                $PaymentResult->setResponse(new Response($threeDAuthHtml));

                return $PaymentResult->getResponse();
            } else {
                $this->addSuccess('yamato_payment.mypage.register.success');
